...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def get_runtime() -> AdkApp:
        ...
        if runtime is None:
            ...
            # Monkeypatch project_id to avoid resource_manager_utils.get_project_id auth check
            AdkApp.project_id = lambda self: (
                self._tmpl_attrs.get("project") or "mock-project"
            )

            # Save original default function and mock it to return AnonymousCredentials on failure
            ...
            try:
                ...
                # Restore original google.auth.default
